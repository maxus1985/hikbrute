...
class HikV2(Hik):
    auth_link = "http://{username}:{password}@{ip}:{port}/ISAPI/Security/sessionLogin/capabilities?username={username}"
    ...
    def detect_suitability(self, ip: str, port: str) -> bool:
        ...
        link = self.auth_link.format(ip=ip, port=port, username="dummy", password="dummy")
